...
object CodecSupport {
    /** Picks an encoder that supports the size (prefers hardware); falls back HEVC→AVC and down in resolution. */
    fun choose(preferHevc: Boolean, width: Int, height: Int, fps: Int): EncoderChoice {
        ...
        for ((w, h) in sizes) for (mime in mimes) {
            val candidates = list.filter { info -> info.supportedTypes.any { it.equals(mime, true) } }
            ...
            for (info in candidates) {
                ...
                if (surfaceOk && vc.isSizeSupported(w, h) && runCatching { vc.areSizeAndRateSupported(w, h, fps.toDouble()) }.getOrDefault(true)) {
                    return EncoderChoice(mime, w, h, info.name, info.isHardwareAccelerated)
                }
            }
        }
        throw AppException(ErrorKind.RenderFailure, "이 기기에서 지원하는 영상 인코더를 찾지 못했습니다.")
    }
}
